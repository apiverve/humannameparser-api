...
class HumannameparserValidationException implements Exception {
  final List<String> errors;

  HumannameparserValidationException(this.errors);

  @override
  String toString() => 'HumannameparserValidationException: ${errors.join("; ")}';
}
...
/// Human Name Parser API client.
///
/// For more information, visit: https://apiverve.com/marketplace/humannameparser?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [name] (required) - The human name to parse
class HumannameparserClient {
  final String apiKey;
  ...
  HumannameparserClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/humannameparser',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HumannameparserValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HumannameparserValidationException(errors);
    }
  }

  /// Execute a request to the Human Name Parser API.
  ///
  /// Parameters are validated before sending the request.
  Future<HumannameparserResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HumannameparserException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HumannameparserResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HumannameparserException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HumannameparserException('Resource not found');
      } else {
        throw HumannameparserException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HumannameparserException) rethrow;
      throw HumannameparserException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Human Name Parser API client.
class HumannameparserException implements Exception {
  final String message;

  HumannameparserException(this.message);

  @override
  String toString() => 'HumannameparserException: $message';
}
